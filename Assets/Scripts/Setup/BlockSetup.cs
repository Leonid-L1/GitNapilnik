...
public class BlockSetup : MonoBehaviour
{
    ...
    public void Init(PlatformView platform, Transform combatantContainer)
    {
        ...
        _view.Init(_cubes.Count, transform.position);
        _model = new BlockModel(GetComponent<BoxCollider>(), combatantContainer);
        _presenter = new BlockPresenter(_view, _model, _cubes);
        _presenter.Enable();
    }
